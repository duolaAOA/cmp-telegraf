...
WORKDIR=/opt/cmp/
ENVFILE=/opt/cmp/cmp-telegraf/service/cmp-telegraf-env
...
fetch https://raw.githubusercontent.com/duolaAOA/cmp-telegraf/master/archive.tar.gz cmp-telegraf.tar.gz
untar cmp-telegraf.tar.gz

# Move original directory to get rid of the branch name so that we
# don't have to worry about it in systemd and init.d files.
mv $( ls | grep -e ^cmp-telegraf-.*$ ) cmp-telegraf
...
if exists systemctl && [ "$(cat /proc/1/comm)" = "systemd" ]; then
    ...
    cp -f /opt/cmp/cmp-telegraf/service/cmp-telegraf.service $systemdir/cmp-telegraf.service
    systemctl enable cmp-telegraf
    systemctl daemon-reload
    systemctl restart cmp-telegraf
else
    sed -i "s/^/export /" $ENVFILE
    cp -f /opt/cmp/cmp-telegraf/service/cmp-telegraf-init.sh /etc/init.d/cmp-telegraf
    chmod +x /etc/init.d/cmp-telegraf
    if exists update-rc.d; then
        update-rc.d cmp-telegraf defaults
    else
        chkconfig --add cmp-telegraf
    fi
    sleep 1
    /etc/init.d/cmp-telegraf restart
    sleep 2
fi
